...
const handleAddThing = (selectedElement: HTMLElement) => {
  ...
  entry.push({
    Component: ({ children }) => (
      <Component data-id="internal">{children}</Component>
    ),
    title: entry.length,
  });
...
const PopUpBase = ({ selectedElement }: { selectedElement: HTMLElement }) => {
  ...
  return (
    ...
    >
      ...
      {entries?.map(({ Component, title }, index) => (
        <Component key={index}>{title}</Component>
      ))}
    </div>
  );
};
...
const createSelectionListener = (
  selectElement: (elem: HTMLElement) => void
) => {
  function clickListener(event: MouseEvent) {
    if (
      event.target &&
      (event.target as HTMLElement).dataset?.id !== "internal"
    ) {
...
const PendoDevMode = () => {
  ...
  >();
  const clickSelectionListener = useRef(
    createSelectionListener(setSelectedElement)
  );
  const [PopUpPortal, setPopUpPortal] = useState<
  ...
  useEffect(() => {
    if (selectedElement) {
      const ps = createPortal(
        <PopUpBase selectedElement={selectedElement} />,
        document.body
      );
      setPopUpPortal(ps);
    }
  ...
  return (
    ...
    >
      <h1>This is a dev mode</h1>
      <button onClick={() => setIsOpen(false)}>Close dev mode</button>
      <button
        style={{
          position: "fixed",
          transition: "transform .5s",
