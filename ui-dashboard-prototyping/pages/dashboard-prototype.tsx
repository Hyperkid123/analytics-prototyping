...
const JourneyIndicator = ({ data }: { data?: { name: string; value: number; }[] }) => {
  if(!data) {
    return null
  }
  return (
...
const JourneyLastStep = ({ data }: { data: {[key: string]: number} }) => {
  const parsedData = Object.entries(data).map(([name, value]) => ({ name, value }))
  return (
    <div>
    <Typography variant="h4" gutterBottom>
...
const EventActivity = () => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/event/activity?${params.toString()}`).then(r => r.json()).then(({ events }: { events: {timestamp: number}[] }) => {
      const groupedEvents = events.reduce<{[key: number]: number}>((acc, curr) => {
...
const ActivityHeatmap = ({ data }: {
  ...
  const maximumAcitvity = Object.entries(data).reduce<number>((acc, [, dayData]) => {
              ...
              heatmap: {
                ...
                colorScale: {
                  ranges: colorScale.map((color, index) => ({
                    from: index * step,
                    to: index + 1 === colorScale.length ? Number.MAX_SAFE_INTEGER : (index + 1) * step,
                    name: `${index + 1 === colorScale.length ? maximumAcitvity : Math.trunc(index * step) + 1} users`,
                    color
                  }))
                }
              }
...
const DashboardPrototype = () => {
  const [data, setData] = useState<{ name: string; value: number; }[] | undefined>(undefined)
  ...
  useEffect(() => {
    fetch('/api/event/active/users').then(r => r.json()).then(({ users }: { [key: string]: number }) => {
      setActiveUsersCount(Object.keys(users).length)
    })
    fetch('/api/event/users/heatmap').then(r => r.json()).then(({ heatmap }: {
        heatmap: {[key: number]: {
          count: number,
          activeSessions: string[]
        }[]
    ...
    fetch('/api/event/journeys?journey=journey-events').then(r => r.json())
    .then(({ journeys }: { journeys: {[key: string]: {type: string, journeyStep: string}[]} }) => {
      const lastSteps = Object.values(journeys).map(group => group.slice(group.length - 1)).flat().reduce<{[key: string]: number}>((acc, curr) => {
        if(curr.type === 'journey-start' || curr.type === 'journey-finish' || curr.type === 'journey-cancel') {
          return acc[curr.type] ? {...acc, [curr.type]: acc[curr.type] += 1} : {...acc, [curr.type]: 1}
        }
    ...
    setLastSteps(lastSteps)
      const data = Object.values(journeys).reduce((acc, curr) => {
        let resolved = false;
        curr.forEach(({type}) => {
      ...
      setData(Object.entries(data).map(([key, value]) => ({
        name: key,
        value
      })))
    })
