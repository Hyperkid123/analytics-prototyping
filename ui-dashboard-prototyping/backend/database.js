...
const getJourneyEvents = (name) => {
  ...
  const journeys = data.events.filter(entry => entry?.type.match(/^journey-/) && entry?.journeyName === name)
...
getEventsByTime = (startTimestamp, endTimestamp) => {
  ...
  let startIndex = 0
  if(startTimestamp) {
    const firstEventIndex = data.events.findIndex(({ timestamp }) => timestamp > startTimestamp)
...
const generateEmptyHeatMap = () => [...Array(7)].reduce((acc, _, index) => {
  return {
    ...acc,
...
const getUserActivityHeatmap = (startTimestamp, endTimestamp) => {
  ...
  const activeByDays = events.reduce((acc, curr) => {
    ...
    return {...acc, [eventDay]: acc[eventDay].map((val, index) => index === eventHour ? sessionDatapoint : val)}
